...
class _BoardState extends State<Board> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_initialized == false || taskNotifier.wasATaskCreated) {
      setState(() {
        _initialized = true;
        _tasks = taskController.fetchUserTasks(session.getSessionUserId() ?? -1,
            boardId: boardNotifier.currentBoard!.id);
      });
    }
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(boardNotifier.currentBoard == null
            ? 'Unknown Taskboard'
            : boardNotifier.currentBoard!.name),
        actions: [
          MenuAnchor(
